...
/*Caso 3: el sensor B mide una distancia superior a 80mm, por ende el robot está andando en diagonal*/
  if (DistanceC_mm < 30 && DistanceB_mm > 30) {
        //El motor izquierdo debe ir más rápido para "enderezar" el robot 
        analogWrite(kMotorLP, 25); 
        analogWrite(kMotorLN, 0); 
        analogWrite(kMotorRP, 40); 
        analogWrite(kMotorRN, 0);
        Serial.print("caso_3 = ");
        Serial.println(DistanceC_mm, DEC);
        //delay(1000);
...
/*Caso 4: Definicion de "Esquina", los 3 sensores miden una distancia inferior a la establecida*/ 
  if (DistanceA_mm < 85 && DistanceB_mm < 130 && DistanceC_mm < 130) {
    while(DistanceA_mm<85){ 
      //Como siempre seguimos la pared derecha con los sensores laterales, hace un giro hacia la derecha 
...
/*Caso 5: el sensor A mide una distancia inferior a la establecida*/
/*  if (DistanceA_mm < 20) {
    while(DistanceA_mm<20){ 
      //Los motores deben ir para atras 
      analogWrite(kMotorLP, 20); 
      analogWrite(kMotorLN, 1); 
      analogWrite(kMotorRP, 20); 
      analogWrite(kMotorRN, 1);
